...
// to it. Called once per agent at startup. The file is never removed
// — it lives for the lifetime of the squad0 process so that any
// subsequent session (runSession, DirectSession, fix-up, learnings
// flush) can read it with an absolute --mcp-config path regardless
// of cwd, without racing worktree cleanup or a stale cached path.
func (orch *Orchestrator) ensureAgentMCPConfig(agentInstance *agent.Agent, baseDir string) {
	...
	agentDir := filepath.Join(baseDir, string(agentInstance.Role()))
